...
def get_sahd_sar(args,trainset, network):
        """returns the score k of a network"""
        neuron_type = 'LIFNode'
        search_batchsize=args.search_batch_size
        network=network.eval()
        ...
        with torch.no_grad():
            ...
            for _ in range(repeat):
                ...
                outputs = network(inputs)
                spiking_activity = (network.spikes / network.neurons) / args.batch_size
                sar_s.append(spiking_activity)
                s.append(logdet(network.K / (network.num_actfun)))
        del inputs
        del outputs
        torch.cuda.empty_cache()
        return np.mean(s), np.mean(sar_s)
...
def prune_func_rank_dvs(xargs, arch_parameters, trainset, val_loader,search_space,network, precision=10, prune_number=1):
    INF=1000
...
def prune_func_rank_sar_sahd(xargs, arch_parameters, trainset, val_loader,search_space,network, precision=10, prune_number=1):
    INF=1000
        ...
        for idx_edge in range(len(arch_parameters[idx_ct])):
            ...
            for idx_op in range(len(arch_parameters[idx_ct][idx_edge])):
                if alpha_active[idx_ct][idx_edge, idx_op] > 0:
                    ...
                    for _ in range(repeat):
                        k,sar = get_sahd_sar(xargs,trainset, network)
                        #sar = get_SAR(xargs,val_loader,network)
                        sar_delta.append(sar)
                        k_delta.append(k)
                    k_all.append([np.mean(k_delta), (idx_ct, idx_edge, idx_op)])
                    sar_all.append([np.mean(sar_delta), (idx_ct, idx_edge, idx_op)])
                    network.zero_grad()
                    pbar.update(1)
    ...
    sar_all = sorted(sar_all , reverse= False)
    for idx, data in enumerate(sar_all):
        if idx == 0:
            rankings[data[1]].append(idx)
        else:
            if data[0] == sar_all[idx-1][0]:
                # same #Regions as previous
                rankings[data[1]].append(rankings[sar_all[idx-1][1]][1])
            else:
                rankings[data[1]].append(rankings[sar_all[idx-1][1]][1]+1)

    rankings_list = [[k, v] for k, v in rankings.items()] # list of [(cell_idx, edge_idx, op_idx), [k_rank]]
    rankings_sum=sorted(rankings_list, key=lambda tup: sum(tup[1]), reverse=False)
    edge2choice = {}  # (cell_idx, edge_idx): list of (cell_idx, edge_idx, op_idx) of length prune_number
    for (cell_idx, edge_idx, op_idx), [k_rank,sar_rank] in rankings_sum:
        if (cell_idx, edge_idx) not in edge2choice:
            edge2choice[(cell_idx, edge_idx)] = [(cell_idx, edge_idx, op_idx)]
        elif len(edge2choice[(cell_idx, edge_idx)]) < prune_number:
            edge2choice[(cell_idx, edge_idx)].append((cell_idx, edge_idx, op_idx))
